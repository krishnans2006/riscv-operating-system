...
# void _thread_startup()
#
# Description:
#     Sets up the arguments for a newly spawned thread, calls its entry function, and calls the exit function when it returns.
#
# Inputs:
#     None
#
...
#
_thread_startup:
        # This function is called from _thread_swtch, if it's the first time the thread is switched to
        # Move arguments out of s0-s7 into a0-a7. Also, extract entry() and running_thread_exit()
        mv      a0, s0
        mv      a1, s1
        mv      a2, s2
        mv      a3, s3
        ...
        # The thread can freely overwrite t and a registers, but not s registers (they're callee-saved)
        # So, we should leave entry() in s8 and running_thread_exit() in s11

        # Call entry(a0, a1, a2, a3, a4, a5, a6, a7)
        jalr    s8

        # When the thread returns, call running_thread_exit()
        jalr    s11
